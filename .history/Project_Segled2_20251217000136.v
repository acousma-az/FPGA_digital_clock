...
parameter LONG_PRESS_THRESHOLD = 26'd25_000_000; // 0.5s 长按触发阈值
parameter REPEAT_INTERVAL = 24'd5_000_000;       // 0.1s 连发间隔

reg k1_sync0, k1_sync1, k1_level, k1_level_prev; reg [19:0] k1_cnt; reg mode_pulse;
reg k2_sync0, k2_sync1, k2_level, k2_level_prev; reg [19:0] k2_cnt; reg inc_pulse;

// 长按快调相关寄存器
reg [25:0] long_press_cnt;  // 长按计时器
reg [23:0] repeat_cnt;      // 连发间隔计时器
reg long_press_active;      // 长按激活标志

// 模式状态机: 0=正常运行, 1=调秒, 2=调分, 3=调时
reg [1:0] set_mode;
...
always @(posedge CLK_50M or negedge RST_N) begin
    if(!RST_N) begin
        k1_cnt <= 20'd0; k1_level <= 1'b1; k1_level_prev <= 1'b1; mode_pulse <= 1'b0;
    end else begin
        mode_pulse <= 1'b0;
        if(k1_sync1 == k1_level) begin
            ...
            if(k1_cnt >= DB_CNT_MAX-1) begin
                ...
                k1_level <= k1_sync1;
                k1_cnt <= 20'd0;
                if(k1_level_prev==1'b1 && k1_sync1==1'b0) mode_pulse <= 1'b1; // 按下
            end else begin
                k1_cnt <= k1_cnt + 20'd1;
            end
        end
...
always @(posedge CLK_50M or negedge RST_N) begin
    if(!RST_N) begin
        k2_cnt <= 20'd0; k2_level <= 1'b1; k2_level_prev <= 1'b1;
    end else begin
...
// KEY2 长按快调逻辑：单击产生一次脉冲，长按0.5s后每0.1s连发
always @(posedge CLK_50M or negedge RST_N) begin
    if(!RST_N) begin
        inc_pulse <= 1'b0;
        long_press_cnt <= 26'd0;
        repeat_cnt <= 24'd0;
        long_press_active <= 1'b0;
    end else begin
        inc_pulse <= 1'b0;
        
        if(k2_level == 1'b0 && set_mode != 2'd0) begin  // 按键按下且在调整模式
            if(!long_press_active) begin
                // 还未进入长按模式
                if(long_press_cnt >= LONG_PRESS_THRESHOLD - 1) begin
                    long_press_active <= 1'b1;
                    repeat_cnt <= 24'd0;
                    inc_pulse <= 1'b1;  // 进入长按时产生一次脉冲
                end else begin
                    long_press_cnt <= long_press_cnt + 26'd1;
                end
            end else begin
                // 已进入长按模式，连发
                if(repeat_cnt >= REPEAT_INTERVAL - 1) begin
                    repeat_cnt <= 24'd0;
                    inc_pulse <= 1'b1;
                end else begin
                    repeat_cnt <= repeat_cnt + 24'd1;
                end
            end
        end else begin
            // 按键松开，复位长按状态
        ...
        // 单击检测（按下瞬间产生脉冲）
        if(k2_level_prev == 1'b1 && k2_level == 1'b0 && set_mode != 2'd0)
            inc_pulse <= 1'b1;
    end
end

// 模式切换状态机: 0->正常, 1->调秒, 2->调分, 3->调时, 再按回0
always @(posedge CLK_50M or negedge RST_N) begin
    if(!RST_N)
        set_mode <= 2'd0;
    else if(mode_pulse)
...
begin
	if(!RST_N)	
        cnt_1s <= 28'd0;
    else if(set_mode != 2'd0)  // 调整模式下暂停走时
        cnt_1s <= 28'd0;
    else if(cnt_1s == SET_TIME_1S - 1)
        cnt_1s <= 28'd0;
    else
...
begin
    ...
    else if(set_mode == 2'd1 && inc_pulse) begin  // 调秒模式
        if(cnt_s == SIXTY - 1)
            cnt_s <= 6'd0;
        else
            cnt_s <= cnt_s + 6'd1;
    end
    else if(set_mode == 2'd0) begin  // 正常运行模式
        if(cnt_s == SIXTY-1 && cnt_1s == SET_TIME_1S - 1)
            cnt_s <= 6'd0;
        else if(cnt_1s == SET_TIME_1S - 1)
            cnt_s <= cnt_s + 6'd1;
    end
...
begin
	if(!RST_N)	
    ...
    else if(set_mode == 2'd2 && inc_pulse) begin  // 调分模式
        if(cnt_m == SIXTY - 1)
            cnt_m <= 6'd0;
        else
            cnt_m <= cnt_m + 6'd1;
        ...
        if(cnt_m == SIXTY-1 && cnt_s == SIXTY-1 && cnt_1s == SET_TIME_1S - 1)
            cnt_m <= 6'd0;
...
begin
    ...
    else if(set_mode == 2'd3 && inc_pulse) begin  // 调时模式
        if(cnt_h == TWENTYFORE - 1)
            cnt_h <= 6'd0;
        else
            cnt_h <= cnt_h + 6'd1;
    end
    else if(set_mode == 2'd0) begin  // 正常运行模式
        if(cnt_h == TWENTYFORE-1 && cnt_m == SIXTY-1 && cnt_s == SIXTY-1 && cnt_1s == SET_TIME_1S - 1)
            cnt_h <= 6'd0;
